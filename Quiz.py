...
def loopPergunta(qtde: int, nome: str, modo: str): #qtde = Quantidade de questões
    ...
    count = 1 # Contador para indicar o numero da pergunta para visualizacao do jogador

    while True:
        index_pergunta = random.choice(list(perguntas.keys())) # criado para pegar o indice da pergunta
        pergunta = perguntas[index_pergunta] # pega os valores da chave primaria
        ...
        or pergunta["dificuldade"] == "D" and perguntasDadas[2] < maxPerguntas[2]):
            gabaritoRodada.append(pergunta["gabarito"]) # vai servir para gerar o gabarito no final 
            print(f'{count}) {pergunta["titulo"]}') #Mostra o enunciado da pergunta pegando o valor 'titulo' do nosso dicionario
            ...
            if len(list(perguntas.keys())) >= 1: # esse if ira garantir que o comando random.choice() nao pegue perguntas repetidas do dicionario "perguntas" criado no inicio 
                del perguntas[index_pergunta] 
